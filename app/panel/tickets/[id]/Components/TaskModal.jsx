...
const TaskModal = ({openModal, setOpenModal, ticketId}) => {
    ...
    const createTask = async() => {
        try {
            const response = await fetch(
                ...
                {
                    method: "POST",
                    body: JSON.stringify({name, description, ticketId}),
                    headers: {
    ...
    const editTask = async() => {
        try {
            ...
            const data = await response.json();
            const currentTasks = currentTicket.tasks.map(task => 
                task._id === data._id ? data : task);

            const updatedTicket = {...currentTicket};
            updatedTicket.tasks = currentTasks;
            setCurrentTicket(updatedTicket);
                            ...
                            <div className="sm:flex sm:items-start">
                                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                    ...
                                    >
                                        ...
                                        >
                                            {currentTask?._id ? "Guardar Cambios" : "Crear Tarea"}
                                        </button>
